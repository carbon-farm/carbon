...
- **25** database migrations applied to production
...
- **In-app Help and a first-visit guided tour, in English then Telugu.** `/help` (linked in the title bar, open to visitors too) has plain-language sections for the viewer's own role — visitor, member, administrator, vendor, expert, moderator, support agent — with search and FAQs (e.g. "I paid but I am still locked", password/codes). On the first visit to each role's home screen a spotlight tour walks through every menu item and the ID, notifications and payment strip (14 steps for a member, 12 for an administrator, 6 for a visitor, fewer for other staff). It runs once per person per device, skips anything not on screen, works with the keyboard (arrows, Esc), becomes a bottom sheet on phones, and is replayed from Help → "Take the tour". Bump `TOUR_VERSION` in `tour/steps.ts` to show it to everyone again after a big change. Keep `help/content.ts` and `tour/steps.ts` in step with any screen you change.
- **Membership plans, a membership switch, and a changeable UPI ID (Administrator).** *Admin → Membership → Membership plans*: any number of plans, each a price for a length of access (the old single ₹499 became a "Monthly / 30 days" plan; add Quarterly, Yearly… with quick-pick lengths and an optional Telugu name and description). Each plan can be switched On or Off — nothing is deleted, people who already paid keep their days, price changes affect only new payments, a switched-off or unknown plan cannot be bought, and the last plan that is on cannot be switched off while membership is required. With more than one plan, members choose one on the payment screen (the payment remembers its plan, price and days, and the verification queue shows them); the strip says "from ₹…"; the public page lists the plans. A **master switch** ("Is membership required?") turns the requirement off for everyone — checkout and farm advice open to every signed-in member, status shows "not required", no payment strip — and back on; it asks for confirmation and is audited. *Membership → Settings → Where payments go*: the **UPI ID customers pay to** can be changed at any time (validated as name@bank, asks to confirm, new payments use it at once, every change is audited with old and new value), with a **spare UPI ID** kept on file and a **Swap** button, and a warning to change or clear the merchant ID when moving to a different merchant account. Prices are no longer on the Settings page. Older screens keep working (the settings row's single price follows the cheapest plan that is on; a payment started with no plan buys the first plan).
- **Membership expiry reminders.** A member is told **3 days before** their access ends and **again once it has ended**, in English and Telugu, with a link to renew: a notification in the bell, plus a slim "Membership ends in 2 days — Renew" strip under the title bar (also for a free grant that is ending — "Free access ends in … — pay to continue"). Each end date is announced once (the dates last announced are remembered on the subscription; a renewal moves the end date so the next expiry is announced afresh); nobody is nagged while a renewal is awaiting verification, when membership is switched off, or about an expiry more than a week old. It runs as an hourly sweep **and** the moment a member opens their membership status (a sleeping free-tier server skips cron jobs, and an in-app notice only matters when the app is open anyway). A WhatsApp/SMS channel would plug in where the notification is created. Bug caught and fixed while testing live: marking "already sent" with `NOT (column = date)` silently skipped every first-time reminder (SQL NULL rule) — now an explicit null branch, with a regression test.
- **Product polish.** Shop cards show the product photo, or a leaf placeholder when there is none (so the grid always lines up), with a ♥ on every card; vendor "My products" and the administrator's product list show a photo thumbnail (and the vendor list gained search, status filter and sort). **Visitors can heart products without signing in** — kept in the browser, folded into their saved wishlist the moment they log in or register (like the guest cart); the product page now also shows the real hearted state. Note: **none of the 30 current products has a photo yet** — vendors/administrators upload them on the product edit screen.
- **Brand.** The title bar keeps "Organic Carbon Farming" for everyone and now adds a small "· HARIHARAA Group" (Telugu line: "· హరిహర గ్రూప్") beside it; the landing page shows the same group line under the name. It fits on a phone.
- **Grouped main menu that always fits.** The menu under the title bar used to be one long row that ran off small screens (16 links for an administrator). It is now grouped — Members: *Shop* (Marketplace, Cart, My orders, Addresses) and *Farm advice* (Dashboard, Cases, Knowledge, Courses, Soil Testing) plus *My subscription*; Administrators: Admin, *People*, *Shop*, *Membership*, *Content*, *Insights & setup*; Moderators: *Review*, *Content*; roles with five links or fewer stay flat. On a laptop the groups are click-to-open dropdowns (the current page's group is highlighted); on a phone or tablet a **Menu** button opens one panel that fills the screen under the menu bar with every group as a heading and large tap targets, showing the current page's name beside the button. Nothing scrolls sideways and no link is hidden. The guided tour opens the right group for each step. The menu definition is `components/navConfig.ts`.
